...
export default function SketchPlane({
  ...
  const {
    ...
    deleteSketch,
    clearAllSketches,
  } = useDocumentSketches(documentId, dimension, addHistoryStep);
  ...
  const getSnappedPoint = useCallback(
    (intersection: THREE.Intersection): Point3D | null => {
  ...
  const findSketchNearPoint = useCallback(
    (point: Point3D, threshold = 2): DrawingItem | null => {
  ...
  return (
    <>
      ...
      {sketches.map((sketch) => {
        ...
        return ToolRenderer
          ? ToolRenderer(sketch, false, isSelected, isHovered)
          : null;
      })}

      {isActive &&
        currentSketch &&
        currentSketch.points.length >= 2 &&
