...
import {
  ...
  documentAllOperationsAtom,
  isUsingDatabaseAtom,
} from "../../../(protected)/atoms/operations-atoms"
...
export default function ExtrudeControls() {
  ...
  const allOperations = useAtomValue(documentAllOperationsAtom(documentId))
  ...
  const handleExtrude = () => {
    ...
    if (isUsingDatabase) {
      // Database mode: Create extrude operation
      // Find the sketch operation
      const sketchOp = allOperations.find(
        (op) => op.id === extrudeState.selectedSketchId
      )
      ...
      const alreadyExtruded = allOperations.some(
        (op) =>
          op.type === 'extrude' &&
          (op.parameters as { sketchOperationId: string }).sketchOperationId ===
      ...
      const extrudeOp: Operation = {
        id: Date.now().toString(),
        sequence: 0, // Will be set by server
        type: 'extrude' as const,
        parameters: {
          sketchOperationId: extrudeState.selectedSketchId,
          depth: extrudeState.depth,
          direction: 'positive',
          color: (sketchOp.parameters as { color?: string }).color,
        },
        dependencies: [extrudeState.selectedSketchId],
        createdAt: new Date(),
        updatedAt: new Date(),
      }

      // Add to pending operations (will be auto-saved)
      setPendingOps((prev) => [...prev, extrudeOp])
      ...
      for (const dim of ['x', 'y', 'z'] as const) {
        const sketch = documentSketches[dim].find(
          (s) => s.id === extrudeState.selectedSketchId
        )
        if (sketch) {
          foundSketch = sketch
          foundDimension = dim
      ...
      const alreadyExtruded = extrudedShapes.some(
        (shape) => shape.sketchId === extrudeState.selectedSketchId
      )
      if (alreadyExtruded) {
        alert('This sketch is already extruded')
        return
