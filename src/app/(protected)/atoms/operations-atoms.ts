 ...
 */
export const documentSketchesFromOperationsAtom = atomFamily(
  (documentId: string) =>
    atom((get) => {
      const operations = get(documentAllOperationsAtom(documentId));

      const sketches: Record<Dimension, DrawingItem[]> = {
        x: [],
        y: [],
        z: [],
      };
      ...
      for (const op of operations) {
        if (op.type === "sketch_line" && "dimension" in op.parameters) {
          ...
          op.type === "sketch_rectangle" &&
          "dimension" in op.parameters
        ) {
          const params = op.parameters as {
            dimension: Dimension;
            startPoint: Point3D;
 ...
 */
export const documentExtrudedShapesFromOperationsAtom = atomFamily(
  (documentId: string) =>
    atom((get) => {
      const operations = get(documentAllOperationsAtom(documentId));
      ...
      return operations
        ...
        .map((op) => {
          ...
          const sketchOp = operations.find(
            (o) => o.id === params.sketchOperationId,
          );
          const dimension =
            (sketchOp?.parameters as { dimension?: Dimension })?.dimension ??
            "z";
